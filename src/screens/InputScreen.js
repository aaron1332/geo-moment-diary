...
import { insertMoment } from '../database/db';
...
export default function InputScreen({ navigation }) {
  ...
  const takePhotoHandler = async () => {
    ...
    } catch (error) { Alert.alert('Error', 'Gagal membuka kamera.'); }
  };

  const removePhoto = (indexToRemove) => setImages(images.filter((_, index) => index !== indexToRemove));

  const saveMomentHandler = async () => {
    if (!title.trim() && !content.trim()) return Alert.alert('Peringatan', 'Catatan tidak boleh kosong sepenuhnya.');
    const finalTitle = title.trim() ? title : 'Catatan Jurnal';
    const photoDataString = JSON.stringify(images);
    const safeLatitude = location ? Number(location.coords.latitude) : 0.0;
    const safeLongitude = location ? Number(location.coords.longitude) : 0.0;
    try {
      await insertMoment(finalTitle, content, todayDate, photoDataString, safeLatitude, safeLongitude);
      navigation.goBack();
    } catch (error) { Alert.alert('Error', 'Gagal menyimpan catatan.'); }
  };
    ...
    <LinearGradient colors={['#FFFFFF', '#F0F8FF', '#74ACDF']} start={{ x: -0.2, y: -0.2 }} end={{ x: 1, y: 1 }} style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={saveMomentHandler}><Text style={styles.saveBtn}>Simpan</Text></TouchableOpacity>
        </View>

        <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollArea}>
          <View style={styles.locationContainer}>
            <Text style={styles.locationText}>{location ? `📍 Terkunci (Akurasi: ±${location.coords.accuracy.toFixed(0)}m)` : '📍 Memindai satelit...'}</Text>
            {location && (
              <View style={styles.miniMapWrapper}>
                <MapView style={styles.miniMap} initialRegion={{ latitude: location.coords.latitude, longitude: location.coords.longitude, latitudeDelta: 0.005, longitudeDelta: 0.005 }} scrollEnabled={false} zoomEnabled={false}>
                  <Marker coordinate={{ latitude: location.coords.latitude, longitude: location.coords.longitude }} pinColor="#74ACDF" />
                </MapView>
              </View>
            )}
          </View>
